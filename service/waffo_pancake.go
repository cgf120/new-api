...
// are exposed separately for self-service flows driven from this gateway UI.
...
func WaffoPancakeBuyerIdentityFromUserID(userID int) string {
	return fmt.Sprintf("fzw-ai-user-%d", userID)
}
...
const (
	defaultWaffoPancakeStoreName   = "ai-gateway-store"
	defaultWaffoPancakeProductName = "ai-gateway-charge-product"
)
...
//
// OnetimeProduct (not SubscriptionProduct) because the gateway has no renewal-
// event handling; Pancake auto-renewing without the gateway extending user
